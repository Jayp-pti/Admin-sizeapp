        ...
        function setCountdown() {
            ...
            let updateCount = setInterval(function () {
                ...
                if (distance < 0) {
                    ...
                    document.querySelector(".comming-soon").innerHTML =
                        "<h1>EXPIRED</h1>";
                }
            }, 1000);
        }
...
$(document).ready(function () {
    ...
    if ($(".add-comment").length > 0) {
        $(".add-comment").on("click", function () {
            $(this)
                .closest(".notes-editor")
                .children(".note-edit-wrap")
                .slideToggle();
        });
...
const delete_templates = document.querySelectorAll('.delete_templates');
if (ViewTemplate) {
    const templates = document.querySelectorAll(".view_templates");
    templates.forEach((template) => {
        template.addEventListener("click", () => {
            ...
            var headers ={};
            var requestData = {};
            ajaxRequest(`fetch_templates/${id}`, method, headers, requestData, function (error, response) {
                if (error) {
                    console.error('Error:', error);
                } else {
                   
                var fieldValues = JSON.parse(response.field_value);
                fieldValues = JSON.parse(fieldValues);
                console.log(fieldValues);
                var tableHtml = '<table class="table table-bordered mb-0">';
                tableHtml += "<thead><tr>";

                if (fieldValues[0] && typeof fieldValues[0] === 'object') {
                    Object.keys(fieldValues[0]).forEach(function (key) {
                        tableHtml += "<th>" + key + "</th>";
                    });
                }
                tableHtml += "</tr></thead>";
                ...
                var image =  `<img class="rounded mx-auto mb-2 d-block" src="storage/${response.image}" alt="${response.name}" width="500">`;
                var htmldata = `<div class="modal-header border-0 custom-modal-header">
                                ...
                                <div class="modal-body custom-modal-body">
                                    <div class="card-body">
                                        <div class="row" style ="align-items: center;">
                                            ${response.image_position == 'top' && response.image != null?  image :'' }
                                            ${response.image_position == 'left' && response.image != null ? `<div class ="col-md-4">${image}</div>`:''}
                                            <div class = "col-md-${response.image_position == 'left' || response.image_position == 'right'? 8:12}">
                                                <div class="table-responsive">
                                                    ${tableHtml}
                                                </div>
                                            </div>
                                            ${response.image_position == 'right' && response.image != null  ? `<div class ="col-md-4">${image}</div>`:''}
                                            </div>
                                        </div>
                                    <div class="row">
                                    ${response.image_position == 'bottom' && response.image != null ? (`<div class="col-12"> ${image}</div>`)  : ''}
                                        <div class="col-12">
                                ...
                                document.getElementById("Dataupdated").innerHTML = '';
                document.getElementById("Dataupdated").innerHTML = htmldata;
                }
            });
        });
    });
...
const removeTemplate = document.querySelector("#remove-template");
delete_templates.forEach(element => {
    element.addEventListener('click', ()=> {
        const tempid = element.getAttribute('data-id');
        removeTemplate.setAttribute('data-id', tempid);
    })
});

if(removeTemplate) {
    removeTemplate.addEventListener('click', ()=> {
        const tempremove_id = removeTemplate.getAttribute('data-id');
        if(tempremove_id) {
            var method = 'DELETE';
            var headers = {
            ...
            var requestData = {};
            ajaxRequest(`size_charts/${tempremove_id}`, method, headers, requestData, function (error, response) {
                if (error) {
                    console.error('Error:', error);
                } else {
                    var elementToRemove = document.getElementById('template_' + tempremove_id);
                    if (elementToRemove) {
                        if(response.length === 0) {
                            var htmldes = `<p class="card-text text-center">Data items is not available</p>`;
                            elementToRemove.insertAdjacentHTML('beforebegin', htmldes);
                        }
...
function ajaxRequest(url, method, headers, data, callback) {
